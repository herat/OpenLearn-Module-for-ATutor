	...
	class Update {
		...
		function parse($date, $urlforrepo,$db,$prefix) {
			...
			if (count($members) > 0) {
				...
				$qry = "SELECT * FROM " . $prefix . "ol_search_open_learn";
				...
				foreach ($members as $member) {
					...
					if (mysql_num_rows($result) > 0) {
						...
						$qry = "UPDATE " . $prefix . "ol_search_open_learn SET DATESTAMP='" . $member['datestamp'] . "' , CATALOG='" .
						...
						$qry = 'INSERT INTO ' . $prefix . 'ol_search_open_learn VALUES (' . $index . ',"' . $member['identifier'] . '","' .
			...
			if ($tmp != "Failed") {
				$qry = "UPDATE " . $prefix . "config SET value=CURDATE() WHERE name='ol_last_updation'";
				$feedback = array('OL_DB_UPDATED');
				//$msg->addFeedback($feedback);
				mysql_query($qry, $db);
				$qry = "UPDATE " . $prefix . "config SET value='".date('H:i:s')."' WHERE name='ol_last_time'";
				mysql_query($qry, $db);
			} 
			else {
